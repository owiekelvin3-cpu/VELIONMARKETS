...
export function AuthBrandPanel() {
  ...
  return (
    <div className="relative hidden min-h-screen flex-col justify-between overflow-hidden bg-gradient-to-br from-charcoal via-void to-void p-10 lg:flex lg:w-[52%] xl:w-[55%] xl:p-14">
      <img
        src={IMAGES.hero.office}
        alt=""
        className="pointer-events-none absolute inset-0 h-full w-full object-cover opacity-[0.14]"
      />
      <div className="absolute inset-0 bg-gradient-to-br from-void/30 via-void/85 to-void" />
      ...
      >
        ...
        <h1 className="font-display text-4xl font-bold leading-[1.1] tracking-tight text-foreground xl:text-5xl">
          {t("auth.panelTitle1")}
          <br />
          <span className="text-gradient-emerald">{t("auth.panelTitle2")}</span>
        </h1>
        <p className="mt-6 text-base leading-relaxed text-muted xl:text-lg">
        ...
        <div className="mt-12 grid grid-cols-3 gap-4">
          {stats.map((stat, i) => (
            ...
            >
              ...
              <p className="mt-1 text-xs leading-snug text-muted">{t(stat.labelKey)}</p>
            </motion.div>
          ))}
        </div>
      </motion.div>
