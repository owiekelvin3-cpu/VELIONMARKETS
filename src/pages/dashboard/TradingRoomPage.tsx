...
export default function TradingRoomPage() {
  ...
  const handleOrder = async (side: "buy" | "sell", amountUsd: number) => {
    ...
    if (err) {
      ...
      setMessage(
        side === "buy"
          ? t("trading.orderPlacedBuy", { pair: pair.label, amount: formatCurrency(amountUsd) })
          : t("trading.orderPlacedSell", { pair: pair.label, amount: formatCurrency(amountUsd) })
      );
      setMessageOk(true);
      loadAccount();
    }
  ...
  return (
    <div className="space-y-4">
      <PageHeader
        ...
        subtitle={t("trading.subtitleSimple")}
        actions={
          <>
      ...
      {/* Step strip — what to do */}
      <div className="surface-muted grid gap-2 p-3 text-xs text-muted sm:grid-cols-3 sm:gap-3 sm:p-3.5">
        <p><span className="font-semibold text-foreground">1.</span> {t("trading.stepPick")}</p>
        <p><span className="font-semibold text-foreground">2.</span> {t("trading.stepAmount")}</p>
        <p><span className="font-semibold text-foreground">3.</span> {t("trading.stepBuySell")}</p>
      </div>
      ...
      <div className="surface-panel p-4">
        ...
        <div className="flex flex-col gap-3 border-t border-border pt-4 sm:flex-row sm:items-end sm:justify-between">
          <div>
            <p className="text-xs text-muted">{t("trading.livePrice")}</p>
            <div className="mt-1 flex flex-wrap items-baseline gap-3">
              ...
              {ticker && (
                <span className={cn(
                  ...
                  {isUp ? "+" : ""}{ticker.priceChangePercent.toFixed(2)}% {t("trading.today")}
                </span>
              )}
            </div>
          </div>
          <div className="inline-flex gap-0.5 rounded-lg border border-border bg-secondary/40 p-0.5 self-start sm:self-auto">
            {INTERVALS.map((iv) => (
              <button
                key={iv.id}
                type="button"
