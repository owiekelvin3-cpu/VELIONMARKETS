  ...
  return (
    <div className="mx-auto max-w-2xl">
      <DepositPageHeader
        ...
        subtitle={t("deposits.giftCardFormDesc")}
        backTo="/dashboard/deposits/gift-card"
      />
          ...
          <FadeIn>
            <div className="rounded-2xl border border-border bg-secondary/50 p-5 sm:p-6">
              ...
              <form onSubmit={handleSubmit} className="space-y-5">
                <div className="grid gap-4 sm:grid-cols-2">
                  <div>
                    <Label htmlFor="amount" className="text-sm text-foreground">
                      {t("deposits.cardValueLabel", { currency: accountCurrency })}
                ...
                {message && (
                  <p className={cn("text-sm", success ? "text-emerald" : "text-red-400")}>{message}</p>
                )}

                <button
                  type="submit"
                  disabled={loading || !frontImage}
                  className="h-12 w-full rounded-xl bg-emerald text-sm font-semibold text-white transition-colors hover:bg-emerald-glow disabled:cursor-not-allowed disabled:opacity-50"
                >
